...
if not ADMIN_KEY:
    ...
    ADMIN_KEY = _secrets.token_hex(24)
...
def agent_token():
    ...
    Requires a valid user identity (X-Forge-User-Id header) to prevent
    unauthenticated access to the agent token.
    """
    uid, _ = _get_identity()
    if not uid:
        return jsonify({"error": "user_id_required", "token": None, "available": False}), 401
