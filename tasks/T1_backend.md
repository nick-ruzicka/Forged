...
CYCLE 17 DEMOLITION UNBLOCKED (2026-04-16): Prompt stack was demolished in commits `c0b2b9e` + `837ed88` plus migration `008_drop_prompt_stack.sql` (drops agent_reviews + tool_versions + runs tables; drops tools.system_prompt/hardened_prompt/input_schema/model/max_tokens/temperature/output_type/output_format/reliability_score/safety_score/complexity_score/verified_score/data_sensitivity/tool_type/workflow_steps/last_run_at columns). **Tasks below that reference `tool_versions.status` (T4 self-healer blocker), `runs` reads (line 55 GET /api/runs/:id), `agent_reviews` projection (line 56 /agent/review), or the progress_pct/review_tokens_used/stage_failed migration-002 additions are NOW OBSOLETE** — these columns/tables no longer exist in v2. The 002 HUMAN-RESCUE block below is **NO LONGER ACTIONABLE** — apply `008_drop_prompt_stack.sql` first via `venv/bin/python3 scripts/run_migrations.py`, then re-scope this task file against the surviving apps-only schema (tools.app_html/app_type/schedule_cron/schedule_channel/container_mode/image_tag + app_data + skills + eval_runs + forge_data_reads + sandbox-related columns). Salvageable legacy tasks: forge_migrations tracking table (line 58), Redis rate limiter for /api/apps/<id>/data/<key> paths (line 59, repurposed from executor.py which no longer exists), GET /api/tools/:id/instructions{.pdf} (lines 52-53, still valid for apps). HUMAN-OPERATOR: strike invalidated items and re-queue this file's v3 block.
