...
CYCLE 17 DEMOLITION UNBLOCKED (2026-04-16): **TERMINAL PARTIALLY INVALIDATED.** `celery_app.py` + `scripts/start_worker.sh` + `scripts/start_beat.sh` survive in the repo, but `agents/tasks.py` was deleted in commit `837ed88` along with the rest of `agents/`. That file held `run_pipeline_task` and `self_heal` — the only Celery tasks this terminal wired. **Cycle 5 tasks below that retry/queue/timeout/DLQ the pipeline task (lines 34-39) are OBSOLETE** because the pipeline task no longer exists. Salvageable Cycle 5 items: result_backend config (line 33 — generic Celery hygiene), priority queues (line 36 — repurposable for `forge_sandbox.tasks.hibernate_idle` which still exists), flower service (line 40 — Celery admin UI independent of task mix), autoscale worker flags (line 41), healthcheck task (line 42). HUMAN-OPERATOR ACTION: re-scope this file around the surviving Celery tasks (`forge_sandbox.tasks.hibernate_idle` + any new apps-era scheduled tasks — T1_app_platform Cycle 7 line 48 proposes `run_scheduled_app` which would be the natural v2 pick up). Do not touch the agents-pipeline retry/DLQ tasks below until a new pipeline-or-equivalent exists to protect.

LEGACY UNBLOCKED (pre-demolition, partially obsolete): All 10 tasks stay inside T1_NEW ownership (celery_app.py, agents/tasks.py, scripts/start_*.sh, docker-compose.yml additions only). Zero cross-terminal dependency — Redis + Celery worker already live per Cycle 1 smoke test. SPEC references: line 55-60 (Celery never runs in Flask), line 380 (pipeline ALWAYS dispatched to Celery), line 603-610 (Celery Beat every 6h). Suggested pick order: result backend FIRST (unblocks task-status polling) → retry policy → progress signals → priority queues → timeout enforcement → DLQ → autoscale → caching → flower → health endpoint.
