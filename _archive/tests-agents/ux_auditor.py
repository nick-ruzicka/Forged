"""Continuous visual UX audit agent.
...
Runs once every 30 minutes. Heavy — makes ~18 screenshots and ~9 Claude calls
per cycle. Tune CYCLE_SECONDS and PAGES if cost is an issue.
"""
...
def main() -> None:
    print(f"[ux_auditor] started. Cycle {CYCLE_SECONDS}s. Report: {REPORT_FILE}")
    while True:
        try:
            report = run_cycle()
            if report.get("_ok") is False:
                print(f"[ux_auditor] cycle error: {report.get('error')}")
            else:
                c = report["counts"]
            ...
            print(f"[ux_auditor] cycle crash:\n{traceback.format_exc()}")
